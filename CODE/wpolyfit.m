...
%WEIGHTED POLYFIT Fit polynomial to data.
%   P = WPOLYFIT(X,Y,N,W) finds the coefficients of a polynomial P(X) of
%   degree N that fits the data Y best in a least-squares sense. P is a
%   row vector of length N+1 containing the polynomial coefficients in
%   descending powers, P(1)*X^N + P(2)*X^(N-1) +...+ P(N)*X + P(N+1).
%
%   [P,S] = WPOLYFIT(X,Y,N,W) returns the polynomial coefficients P and a
...
%
%   [P,S,MU] = WPOLYFIT(X,Y,N,W) finds the coefficients of a polynomial in
